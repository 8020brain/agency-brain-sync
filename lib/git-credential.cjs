...
function registerRepo(repoPath, value) {
  if (!repoPath || !fs.existsSync(path.join(repoPath, '.git'))) return false;
  const desired = ['', value];
  const got = spawnSync('git', ['-C', repoPath, 'config', '--local', '--get-all', 'credential.helper'], { encoding: 'utf8' });
  const current = got.status === 0 ? (got.stdout || '').replace(/\n$/, '').split('\n') : [];
  if (JSON.stringify(current) === JSON.stringify(desired)) return true;
  spawnSync('git', ['-C', repoPath, 'config', '--local', '--unset-all', 'credential.helper']);
  spawnSync('git', ['-C', repoPath, 'config', '--local', '--add', 'credential.helper', '']);
  const add = spawnSync('git', ['-C', repoPath, 'config', '--local', '--add', 'credential.helper', value]);
  return add.status === 0;
}
...
function ensureCredentialHelper(config, opts) {
  try {
    ...
    for (const p of profiles) {
      ...
      registerRepo(p.brainPath, helperConfigValue(opts.exePath, helperPath, p.teamSlug));
    }
