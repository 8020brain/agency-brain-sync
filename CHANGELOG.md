...
- **When something needs you, the menu makes it impossible to miss.** The status line and the fix actions are the only live items, with a red marker at the top; everything routine is greyed out until the problem clears; on a Mac the words "Needs attention" sit next to the icon in the menu bar, and on Windows a message rises from the tray icon (even when the icon is tucked behind the ^ arrow) and clicking it opens Help me fix this. Before this the status line was grey like everything else, so a stopped brain looked normal unless you noticed the icon.
