'use strict';
  ...
  var GUIDES={
    ...
    'g-claudemd':{html:'<h2>Setting up your AGENTS.md files</h2><p class="gp-path">docs/scout-guide/your-claude-md.md</p>'
      +'<p><code>AGENTS.md</code> is how you tell your AI assistant how your agency works. It reads the file automatically at the start of every session in this folder. Next to every <code>AGENTS.md</code> sits a short <code>CLAUDE.md</code> holding one line that points at it: that pointer is how Claude finds the instructions, so leave it alone and write in <code>AGENTS.md</code>. If your brain was made before August 2026 it has no <code>AGENTS.md</code> yet, and everything below applies to <code>CLAUDE.md</code> instead; run <code>/agency-update</code> and say yes when it offers the conversion. There are four places these conventions live, and they stack.</p>'
      +'<h3>1. The shared agency AGENTS.md</h3><p>The root file is the one everyone shares: who you are, services, clients, tools, voice, working preferences. Change it and sync, and every teammate picks it up within about a minute. Fill the placeholders by running the <code>/agency-brain-context-setup</code> prompt in Claude Code. You have write access; team members don\'t.</p>'
      +'<h3>2. Your personal folder</h3><p><code>personal/</code> is your private scratch space for notes, todos and drafts. It stays on your machine and never syncs, so it isn\'t backed up either: don\'t keep the only copy of anything there. It holds private <em>files</em>, not instructions.</p>'
      +'<h3>3. Your private CLAUDE.local.md</h3><p>For private <em>rules</em>, instructions loaded only on your machine, use <code>CLAUDE.local.md</code> at the brain root. It keeps its name and has no <code>AGENTS</code> twin, because it\'s your own identity file rather than part of the shared pair.</p>'
      +'<h3>4. Subfolder AGENTS.md files</h3><p>A <code>clients/acme/AGENTS.md</code> carries that client\'s tone and rules. Client-specific rules belong with the client, not the root file.</p>'
      +'<h3>Using ChatGPT instead of Claude?</h3><p>Open the brain folder as a Codex project in the ChatGPT desktop app and it reads the same <code>AGENTS.md</code> and runs the same skills. Open it as a Work task instead and it won\'t: there the folder is an attachment rather than the project, so none of your instructions load.</p>'},
    ...
    'g-owner-setup':{html:'<h2>Owner setup walkthrough</h2><p class="gp-path">onboarding-owner.md</p>'
      ...
      +'<ol><li><b>Create the brain.</b> The app\'s setup wizard creates your agency\'s GitHub repo from the template when you set up; you don\'t do this by hand.</li><li><b>Install the Agency Brain app</b> from ads2ai.com/downloads and let its wizard clone the brain to your machine.</li><li><b>Connect Cowork</b> by pointing it at the folder the app keeps in sync.</li><li><b>Fill your agency context</b> with the tune-brain prompt or <code>/agency-brain-context-setup</code>.</li><li><b>Add your people</b> from your Dashboard here (Add member): scouts first, then team.</li></ol>'
