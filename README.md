...
- Watcher: classifies repo state on every tick; auto-recovers from diverged states via a plain merge that keeps both sides (a conflicting file gets a sidecar copy rather than being overwritten); never stashes, never rebases the working tree. Push lane and pull lane are separated (debounce owns push; interval owns pull and only pushes as a safety net when no debounce is pending). The role-aware path filter uses a deny model: owners and scouts have no path restriction, while team members are blocked from root-level files and root dotpaths (`.claude/`, `.team-config/`, `.github/`, and similar) but can write into any content folder, including folders created after install. A blocked or oversized file is held locally while the rest of the sync keeps flowing (surfaced in `state.held[]` for the tray); the watcher only STOPs for offline or push-rejected situations and for a rebase the user started manually.
- Wizard (`src/wizard.html`): an 11-scene flow (welcome, email, OTP, pick agency, have-a-repo, adopt-existing-brain, machine check, clone, surface selection, business context, done). There are two ways in: a 6-character invite code, or email plus a 6-digit OTP for owners and scouts. It also covers solo/personal mode and the in-place solo→agency flip. In agency mode, the clone step detects when the target folder is already a clone of this repo and adopts it in place rather than re-cloning; unrelated non-empty folders still block. Demo mode (code `DEMO01`) seeds a placeholder folder so the flow can be walked end-to-end without the backend. State emission via a `STATE_FILE` env var lets the tray icon reflect a third "needs attention" state when the watcher reports STOP.
